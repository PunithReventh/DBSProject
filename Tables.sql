...
testType VARCHAR(1) CHECK (testType IN ('Internship','Full-time','Both')),
...
jobType VARCHAR(1) CHECK (jobType IN ('Internship', 'Full-time')),
...
CONSTRAINT FK_OFFER_JOBID FOREIGN KEY (jobId) REFERENCES JOB(jobId),
CONSTRAINT FK_OFFER_COMPID FOREIGN KEY (companyId) REFERENCES COMPANY(companyId)
);
